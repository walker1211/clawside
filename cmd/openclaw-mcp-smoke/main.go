...
func run(args []string, stdout, stderr io.Writer) error {
	...
	fs.SetOutput(stderr)
	fs.StringVar(&defaults.Profile, "profile", defaults.Profile, "smoke profile: "+supportedProfileValues+"; private-coordination runs truth-plane coordination rehearsal with sender, MCP startup, registration, and truth-plane evidence boundaries; external-runtime-evidence validates a read-only evidence file without sender, MCP startup, or delivery")
	...
	fs.StringVar(&defaults.OpenClawExternalRuntimeEvidencePath, "openclaw-external-runtime-evidence", defaults.OpenClawExternalRuntimeEvidencePath, "read-only JSON file containing extracted external runtime evidence to validate")
