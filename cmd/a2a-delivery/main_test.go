...
func TestRunUsesMainBuiltInTargetAgentMapping(t *testing.T) {
	const (
		jobID  = int64(106)
		chatID = int64(700010)
	)
	...
	if err != nil {
		t.Fatalf("run main built-in mapping delivery: %v", err)
	}
	if !strings.Contains(stdout.String(), `"bot": "main"`) {
		t.Fatalf("expected main bot in output, got %s", stdout.String())
	}
