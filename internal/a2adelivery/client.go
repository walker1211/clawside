...
func (c *SenderClient) GetJob(ctx context.Context, jobID int64) (SenderJob, error) {
	var zero SenderJob
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/jobs/%d", c.baseURL, jobID), nil)
	if err != nil {
		return zero, fmt.Errorf("build /jobs request: %w", err)
	}
	if c.authKey != "" {
		req.Header.Set("Authorization", "Bearer "+c.authKey)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return zero, decodeSenderAPIError(resp)
	}

	var job SenderJob
	if err := json.NewDecoder(resp.Body).Decode(&job); err != nil {
		return zero, fmt.Errorf("decode /jobs response: %w", err)
	}
	return job, nil
}
...
func decodeSenderAPIError(resp *http.Response) error {
	...
	return &senderAPIError{StatusCode: resp.StatusCode, Message: msg}
}
